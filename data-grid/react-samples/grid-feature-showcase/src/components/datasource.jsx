export function makeData(count) {
  ...
  function pickAvoidTriplet(arr, fieldName) {
    ...
    if (data.length >= 2) {
      ...
      const a = prevItems[data.length - 1][fieldName];
      const b = prevItems[data.length - 2][fieldName];
      if (a === b && a === v) {
  ...
  const baseOrderDate = new Date(2026, 8, 10);
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const salesChannel = randChoice(['Web', 'Marketplace', 'Retail', 'Wholesale']);
    const marketRegion = randChoice(['North America', 'Europe', 'APAC', 'LATAM', 'Middle East']);
    const deliveryMode = randChoice(['Ground', 'Air', 'Express', 'Freight']);
    const trackingNumber = `TRK-${randInt(100000, 999999)}`;
    const currency = randChoice(['USD', 'CAD', 'MXN', 'GBP']);
    const isPriorityOrder = priorityVal === 'Critical' || orderStatusVal === 'Ready To Ship';
    const lastUpdated = new Date(orderDate);
    lastUpdated.setDate(orderDate.getDate() + randInt(0, 8));
